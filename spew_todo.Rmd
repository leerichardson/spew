...
## Canadian PUMS
  ...
  SHAPEFILES- Downloaded a Stats canada shapefile that should word out perfectly  
  
  COUNTS- Downloaded a 2011 .zip CSV. Should be able to parse this and link it with the shapefile. 

  PUMS- Need to read in the files, obtain the column names, and understand how to link these together. 

  - Understand which columns are needed for `"place_id"`, `"puma_id"`, "PERSONS", and "SERIALNO" in each one of the data sources. 
  
  - Write the data_group = "none" section of spew, with filepaths to these three sources. 
  
  - Test this on the local computer and then get this running on Olympus!
