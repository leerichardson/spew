...
Finally, once you re-load the package, it's useful to debug it in Interactive mode on Olympus, just to make sure things are working as expected. you can submit an interactive job (ie: Log onto one of the compute nodes) with the command `qsub -I`. To save time, I usually run the US version on State 01 to find bus:
...
in_dir <- "/data/shared_group_data/syneco/input/west/north_america/united_states/01/"
out_dir <- "/data/shared_group_data/syneco/outputs/spew/"
...
We will have one shell script, called `run_spew.sh`, located in the `/data/shared_group_data/syneco/spew` directory. This script will take inputs (for starters, I'm just using `data_group`, `input_dir`, and `output_dir`) and submit a job to Olympus which requests the desired amount of computing time and runs spew. Thus, we can just call the `run_spew.sh` script using the specific inputs, and everything should run smoothly. 
