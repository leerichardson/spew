...
Finally, once you re-load the package, it's useful to debug it in Interactive mode on Olympus, just to make sure things are working as expected. you can submit an interactive job (ie: Log onto one of the compute nodes) with the command `qsub -I -l walltime=3:00:00`. To save time, I usually run the US version on State 01 to find bugs:
...
in_dir <- "/data/shared_group_data/syneco/input/west/south_america/chile"
out_dir <- "/data/shared_group_data/syneco/outputs/spew/temp/"
...
# data_group = "none"
folders = list(pop_table = "/data/shared_group_data/syneco/input/west/north_america/canada/counts/canada_pop_table.csv", 
                     pums = list(pums_h = "/data/shared_group_data/syneco/input/west/north_america/canada/pums/pums_h.csv", 
                                 pums_p = "/data/shared_group_data/syneco/input/west/north_america/canada/pums/pums_p.csv"),  
                     shapefiles = "/data/shared_group_data/syneco/input/west/north_america/canada/shapefiles/canada_shapefiles.shp")

generate_spew(input_dir = in_dir, output_dir = out_dir, folders = folders, 
              parallel = FALSE, data_group = "none", sampling_type = "uniform", 
              convert_count = FALSE, 
              	vars = list(household = c("SERIALNO", "puma_id"), 
				                    person = c("SERIALNO", "puma_id", "EF_ID","CF_ID","PP_ID","ABOID","AGEGRP","AGEIMM","ATTSCH","BFNMEMB","BEDRM","BUILT","CF_RP","CFSTAT",
							"CFSTRUCT","CIP2011","CITIZEN", "CONDO","COW","DIST","DTYPE","DUR","EF_RP","EFDECILE","EFDIMBM","EMPIN","ETHDER",
							"FCOND","FOL","FPTWK","GENSTAT","GROSRT","GTRFS","HDGREE","HHMAINP","HLAEN","HLAFR","HLANO","HLBEN","HLBFR","HLBNO",
							"HRSWRK","IMMSTAT","INCTAX","KOL","LEAVE","LFTAG","LOC_ST_RES","LOCSTUD","LOLICOA","LOLICOB","LOLIMA","LOLIMB",
							"LOLIMMI","LOMBM","LSTWRK","LWAEN","LWAFR","LWANO","LWBEN","LWBFR","LWBNO","MARSTH","MOB1","MOB5","MODE",
							"MRKINC","MTNEN","MTNFR","MTNNO","NAICS","NOCEE","NOCS","NOL","NOS","OCC","OMP","POB","POBF","POBM","POWST",
							"PR","PR1","PR5","PRESMORTG","PRIHM","PWPR","REGIND","RELIGION","REPAIR","ROOM","SEX","SSGRAD","SUBSIDY","TENUR",
							"TOTINC","TOTINC_AT","VALUE","VISMIN","WEIGHT","WKSWRK","WRKACT","WT1","WT2","WT3","WT4","WT5","WT6","WT7","WT8","YRIMM")))
```
