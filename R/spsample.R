...
sample.Line = function(x, n, type, offset = runif(1), proj4string = CRS(as.character(NA)), ...) {
  ...
  # Get the length of each line segment
  ...
  # Get the cumsum of all the lengths of line segments
  csl = c(0, cumsum(lengths))
  maxl = csl[length(csl)]

  # Sample ranfom number from this line segment
  if (type == "random")
    pts = runif(n) * maxl
...
sample.Polygon = function(x, n, type = "random", bb = bbox(x),
  ...
  # If the area is 0, sample from a line instead
  if (area == 0.0) {
  ...
  while (is.null(res) && its < iter) {
    ...
    for (i in seq(along=ptsres)) {
      if (smple[i]) {
        ...
        ptsres[[i]] <- sample.Polygon(x=pls[[i]], n=nnow, type = type, offset = offset, iter=iter)
      }
    }
