...
ipums_list <- spew:::summarize_ipums(output_dir, ipums_fs, doPrint = doPrint, sampSize = sampSize, readFun = read.csv) #data.table::fread)
ipums_sum_list <- ipums_list
hh_list <- ipums_list$hh_sum_list
```
There is/are `r ipums_fs$nLevels` level(s) of nested ecosystems in `r toupper(region)`.
