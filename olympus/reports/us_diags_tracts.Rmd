...
input_dir <- paste0(input_dir, "/")
 ## input_dir <- "/mnt/beegfs1/data/shared_group_data/syneco/spew_1.2.0/americas/northern_america/usa/10/output/output_101/eco"
 ## tract_id <- "10003000200"
library(spew)
library(devtools)
#load_all("~/spew")
cur_dir <- input_dir
...
# STATEFPS to name
fps <- substr(tract_id, 1, 2)
state_name <- spew:::fipsToPlaceName(fps, level = "state", df = us)
co <- substr(tract_id, 1, 5)
co_name <- spew:::fipsToPlaceName(co, level="county", df = us)
## Data 
data(us_pums_sf)
```

# `r toupper(state_name)`; `r toupper(co_name)`; Tract `r substr(tract_id, 6, 11)`
...
region <- substr(tract_id, 6, 11)
varsToSummarize = list(vars_hh = c("NP", "HINCP"), vars_p =c ("AGEP", "RAC1P"))
doPrint <- TRUE
ipums_fs <- summarizeLowestFS(tract_id, output_dir)
sum_level <- 1
sampSize <- 10000
ipums_list <- summarize_us(output_dir, ipums_fs, varsToSummarize = varsToSummarize, doPrint = doPrint, sampSize = sampSize, sum_level = sum_level, threshold_list = us_pums_sf)
...
cbbPalette <- c("#999999", "#E69F00", "#56B4E9",
	"#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")[-1]
